...
class ASGManager:
    ...
    def describe_account_limits_asg() -> dict:
        """
        Dynamically retrieves all asg and launch configuration limits in one
        api call.
        ...
        try:
            ...
            return {'MaxASG':maxASG,
                    'UsedASG':usedASG,
                    'UsedLC':usedLaunchConfigs,
                    'MaxLC':maxLaunchConfigs,
                    'LaunchConfigPercentage':calcPercentageLC,
                    'ASGPercentage':calcPercentageASG,
                    }
    ...
    def limitExecutor(self):
        ...
        if asg:
            print('* {percentage}% Used for Launch Configs, Launch Config Used: '
                  '{used}, Max Launch Configs: {max} {status}'.format(
